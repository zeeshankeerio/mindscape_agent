-- Database Schema for Mindscape Agent Telnyx Messaging App
...
-- Create RLS policies
-- Contacts policy: Users can only see their own contacts
CREATE POLICY "Users can view own contacts" ON contacts
    FOR ALL USING (user_id = current_setting('request.jwt.claims', true)::json->>'sub');

-- Messages policy: Users can only see their own messages
CREATE POLICY "Users can view own messages" ON messages
    FOR ALL USING (user_id = current_setting('request.jwt.claims', true)::json->>'sub');

-- Inbound settings policy: Users can only see their own settings
CREATE POLICY "Users can view own inbound settings" ON inbound_settings
    FOR ALL USING (user_id = current_setting('request.jwt.claims', true)::json->>'sub');

-- Messaging profiles policy: Users can only see their own profiles
CREATE POLICY "Users can view own messaging profiles" ON messaging_profiles
    FOR ALL USING (user_id = current_setting('request.jwt.claims', true)::json->>'sub');

-- Insert default data for development
-- Note: In production, you would use proper user authentication
